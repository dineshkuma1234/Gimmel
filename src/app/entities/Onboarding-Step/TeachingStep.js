...
const router = useRouter();
  ...
  const handleCheckboxChange = (e) => {
    ...
    if (id === "select-deselect-all") {
      const newCheckedState = {};
      const newArr = filterContentMaturity.map((item) => {
        newCheckedState[item.name] = checked;
        return item.name;
  ...
  const handleChangeeducation = (e) => {
    ...
    if (id === "select-deselect-all") {
      const newCheckedState = {};
      const newArr = eduction.map((item) => {
        newCheckedState[item.name] = checked;
        return item.name;
      }); // Select/Deselect all topics
                ...
                <div className="banner-btn-inner">
                  <button
                    className={`btn-border ${
                      selectedval.includes("School staff member")
                        ? "selected"
                        : ""
                    }`}
                  ...
                  <button
                    className={`btn-border ${
                      selectedval.includes("Parent/guardian") ? "selected" : ""
                    }`}
                  ...
                  <ul className="checkbox-group">
                    <Form>
                      <ListGroup>
                        ...
                        {eduction &&
                          Array.isArray(eduction) &&
                          eduction.map((topic, index) => (
                            <ListGroup.Item key={index}>
                              <Form.Check
                                type="checkbox"
                                id={topic.name}
              ...
              <div className="step-button">
                {currentIndex === images.length - 1 ? (
                  <button
                    ...
                    disabled={(currentIndex === 4 && slectedEducation.length === 0)}
                  >
                    Finish
                  </button>
                ) : (
                  <button
                    ...
                    onClick={nextSlide}
                    disabled={
                      (currentIndex === 0 && selectedval.length < 1) ||
